...
// Date     By      Notes
// -----    ------  ---------------------------------------------------------------------------------
// 131114  Pratlk  PBFI-2512, Refactor client windows in component ENTERP
...
// 140704  Hawalk  Bug 116673, Changed view of tblOurId from MANUFACTURER_INFO_OUR_ID to MANUFACT_INFO_OUR_ID_FIN_AUTH (user-company authorization rests
// 140704          in ACCRUL, that cannot be directly referenced in the server - hence a new view).
...
namespace Ifs.Application.Enterp
{
	...
	/// <summary>
	/// </summary>
	[FndWindowRegistration("MANUFACTURER_INFO", "ManufacturerInfo", FndWindowRegistrationFlags.HomePage)]
	public partial class frmManufacturerInfo : cMasterDetailTabFormWindow
	{
		...
		/// <returns></returns>
		public new static frmManufacturerInfo FromHandle(SalWindowHandle handle)
		{
			return ((frmManufacturerInfo)SalWindow.FromHandle(handle, typeof(frmManufacturerInfo)));
		}
		#endregion
		...
		public new SalBoolean FrameStartupUser()
		{
			...
			{
				Sal.SetWindowText(i_hWndFrame, Properties.Resources.HEAD_DetailManufacturerInfo);
                return true;
			}
			#endregion
		}
			...
			using (new SalContext(this))
			{
				...
				using(SignatureHints hints = SignatureHints.NewContext())
				{
					...
					DbPLSQLBlock(cSessionManager.c_hSql, sFullFormName + ".dfsExist :=\r\n" +
						"                                 &AO.Association_Info_API.Association_No_Exist( " + sFullFormName + ".dfsAssociationNo, 'MANUFACTURER' )");
				}
		...
		public new SalBoolean DataSourceSaveCheckOk()
		{
			...
			using (new SalContext(this))
			{
				...
				{
					if (Sal.SendMsg(frmManufacturerInfoAddress.FromHandle(hWndAddress).tblCommMethod, Ifs.Fnd.ApplicationForms.Const.PM_DataSourceIsDirty, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire, Sys.lParam)) 
					{
						if (!(frmManufacturerInfoAddress.FromHandle(hWndAddress).tblCommMethod.CheckDefault())) 
						{
							return false;
						}
					}
				...
				{
					if (Sal.SendMsg(frmManufacturerInfo.FromHandle(i_hWndFrame).TabAttachedWindowHandleGet(picTab.FindName("Name2")), Ifs.Fnd.ApplicationForms.Const.PM_DataSourceIsDirty, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire, Sys.lParam)) 
					{
						if (!(frmManufacturerInfoCommMethod.FromHandle(hWndComMethod).tblCommMethod.CheckDefault())) 
						{
							return false;
						}
					}
		...
		public new SalBoolean DataSourceSaveCheck()
		{
			...
			using (new SalContext(this))
			{
				...
				if (sAddressTabActive == "TRUE") 
				{
					if (((bool)Sal.SendMsg(frmManufacturerInfoAddress.FromHandle(hWndAddress).tblAddressType, Ifs.Fnd.ApplicationForms.Const.PM_DataSourceIsDirty, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire, Sys.lParam)) || (Sal.QueryFieldEdit(frmManufacturerInfoAddress.FromHandle(hWndAddress).dfdValidFrom) || 
					Sal.QueryFieldEdit(frmManufacturerInfoAddress.FromHandle(hWndAddress).dfdValidTo))) 
					{
						if (!(frmManufacturerInfoAddress.FromHandle(hWndAddress).tblAddressType_CheckDefaultAddress())) 
						{
							return false;
						}
                        if (!(frmManufacturerInfoAddress.FromHandle(hWndAddress).tblAddressType_CheckLastAddressType()))
                        {
                            return false;
                        }
					}
        ...
        public virtual new SalNumber TabActivateFinish(SalWindowHandle hWnd, SalNumber nTab)
        {
            ...
            using (new SalContext(this))
            {
                ...
                if (sTabName == "Name2")
                {
                    ...
                    Sal.SetFocus(frmManufacturerInfoCommMethod.FromHandle(hWndInfoCommMethod).tblCommMethod);
                }
            }
            return retVal;
            #endregion
		...
		private void frmManufacturerInfo_WindowActions(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			switch (e.ActionType)
			{
				case Sys.SAM_Create:
					this.frmManufacturerInfo_OnSAM_Create(sender, e);
					break;
				
				case Ifs.Fnd.ApplicationForms.Const.PM_DataSourcePopulate:
					this.frmManufacturerInfo_OnPM_DataSourcePopulate(sender, e);
					break;
				
				
				case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceSave:
					this.frmManufacturerInfo_OnPM_DataSourceSave(sender, e);
					break;
				
                
                case Ifs.Fnd.ApplicationForms.Const.PM_DataRecordDuplicate:
                    this.frmManufacturerInfo_OnPM_DataRecordDuplicate(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataRecordPaste:
                    this.frmManufacturerInfo_OnPM_DataRecordPaste(sender, e);
                    break;
		...
		private void frmManufacturerInfo_OnSAM_Create(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
		...
		/// </summary>
		/// <param name="message"></param>
		private void frmManufacturerInfo_OnPM_DataSourcePopulate(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
		...
		/// </summary>
		/// <param name="message"></param>
		private void frmManufacturerInfo_OnPM_DataSourceSave(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
			if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataSourceSave, Sys.wParam, Sys.lParam)) 
			{
				if (Sys.wParam == Ifs.Fnd.ApplicationForms.Const.METHOD_Execute) 
				{
					...
					{
						this.TabInvalidateData(this.picTab.FindName("Name1"));
					}
					this.bTabName1Edited = Sal.SendMsgToChildren(this.TabAttachedWindowHandleGet(this.picTab.FindName("Name1")), Ifs.Fnd.ApplicationForms.Const.PM_DataSourceIsDirty, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire, Sys.lParam);
        ...
        private void frmManufacturerInfo_OnPM_DataRecordDuplicate(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void frmManufacturerInfo_OnPM_DataRecordPaste(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
		...
		/// <param name="sender">The source of the event.</param>
		/// <param name="e">An <see ref="PPJ.Runtime.WindowActionsEventArgs"/> that contains the event data.</param>
		private void cmbManufacturerId_WindowActions(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			switch (e.ActionType)
			{
				case Ifs.Fnd.ApplicationForms.Const.PM_DataItemNew:
					this.cmbManufacturerId_OnPM_DataItemNew(sender, e);
					break;
			}
			#endregion
		}
		...
		/// </summary>
		/// <param name="message"></param>
		private void cmbManufacturerId_OnPM_DataItemNew(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
			Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemNew, Sys.wParam, Sys.lParam);
			this.cmbManufacturerId.EditDataItemSetEdited();
			e.Return = true;
			return;
			#endregion
		}
