...
#endregion
#region History
// 090902	Hiwilk	Bug 82001, Modified frmTaxOfficeInfoAddress().
...
// 121003   Hecolk  Bug 102095, Merged from APP75
// 121012   PRatlk  Bug 105758, Reverted the changes by Bug 82001
// 131125   PRatlk  PBFI-2551, Client Refactoring in ENTERP.
// 140401   Kagalk  PBFI-4142, Merged Bug 113712, Modified to give a warning when deleting the last address type.
// 160419   reanpl  STRLOC-355, Added handling of new attributes address3, address4, address5, address6
#endregion
...
namespace Ifs.Application.Enterp
{
	...
	/// <summary>
	/// </summary>
	public partial class frmTaxOfficeInfoAddress : cMasterDetailTabFormWindow
	{
		...
		/// Default Constructor.
		/// </summary>
		public frmTaxOfficeInfoAddress()
		{
			// This call is required by the Windows Form Designer.
			InitializeComponent();
            this.MinimumSize = new Size(0, 457);
            // TODO: Should be removed after merge of LCS 75236
		...
		/// <returns></returns>
		public new static frmTaxOfficeInfoAddress FromHandle(SalWindowHandle handle)
		{
			return ((frmTaxOfficeInfoAddress)SalWindow.FromHandle(handle, typeof(frmTaxOfficeInfoAddress)));
		}
		#endregion
		...
		public virtual SalNumber NewAddressTypes()
		{
			...
			{
				lsStmt = "BEGIN\r\n" +
				"		&AO.Tax_Office_Addr_Type_Code_API.Enumerate_Type( :i_hWndFrame.frmTaxOfficeInfoAddress.__sAddrTypes, " + "'" + "TAXOFFICE" + "'" + ");\r\n" +
				"	     END;";
				using(SignatureHints hints = SignatureHints.NewContext())
				{
					hints.Add("Tax_Office_Addr_Type_Code_API.Enumerate_Type", System.Data.ParameterDirection.Output, System.Data.ParameterDirection.Input);
		...
		public virtual SalString DefaultExist()
		{
			...
			using (new SalContext(this))
			{
                DbPLSQLBlock(cSessionManager.c_hSql, ":i_hWndFrame.frmTaxOfficeInfoAddress.sResult := &AO.Tax_Office_Addr_Type_API.Default_Exist (\r\n" + sFullNameTbl + ".tblAddressType_colsTaxOfficeId IN,\r\n" + sFullNameTbl + ".tblAddressType_colsAddressId IN ,\r\n" + sFullNameTbl + ".tblAddressType_colsAddressTypeCode IN )");
		    }
            return sResult;
			#endregion
		}
		...
		public virtual SalNumber SetCountry()
		{
			...
			using (new SalContext(this))
			{
				sCountry = frmTaxOfficeInfoAddress.FromHandle(i_hWndFrame).cmbCountry.Text;
				if (sCountry == Ifs.Fnd.ApplicationForms.Const.strNULL) 
				{
					frmTaxOfficeInfoAddress.FromHandle(i_hWndFrame).cmbCountry.Text = frmTaxOfficeInfo.FromHandle(i_hWndParent).cmbCountry.Text;
					sCountry = frmTaxOfficeInfo.FromHandle(i_hWndParent).cmbCountry.Text;
				}
			}
		...
		public new SalBoolean FrameStartupUser()
		{
			...
			using (new SalContext(this))
			{
				lsAddressLayoutsDbStmt = "&AO.ADDRESS_PRESENTATION_API.Get_All_Layouts(:i_hWndFrame.frmTaxOfficeInfoAddress.sCountry,\r\n" +
				"						:i_hWndFrame.frmTaxOfficeInfoAddress.lsDisplayLayout,\r\n" +
				"						:i_hWndFrame.frmTaxOfficeInfoAddress.lsEditLayout, \r\n" +
				"						:i_hWndFrame.frmTaxOfficeInfoAddress.sCountryCode )";
				lsLovReferenceDbStmt = "&AO.Enterp_Address_Country_API.Set_Lov_Reverence_All(:i_hWndFrame.frmTaxOfficeInfoAddress.sCountryCode,\r\n" +
				"							:i_hWndFrame.frmTaxOfficeInfoAddress.sStateLov,\r\n" +
				"							:i_hWndFrame.frmTaxOfficeInfoAddress.sCountyLov, \r\n" +
                "							:i_hWndFrame.frmTaxOfficeInfoAddress.sCityLov, \r\n" +
				"							:i_hWndFrame.frmTaxOfficeInfoAddress.sZipLov )";
                lsDetailedAddressDbStmt = ":i_hWndFrame.frmTaxOfficeInfoAddress.sDetailAddress := \r\n" +
                "                                               		&AO.Enterp_Address_Country_API.Get_Detailed_Address(\r\n" +
                "                                               		&AO.ISO_Country_Api.Encode(\r\n" +
                "				:i_hWndFrame.frmTaxOfficeInfoAddress.cmbCountry))";
		...
		public virtual SalNumber InitValuesForPopulate()
		{
			...
			{
				lsStmt = "DECLARE\r\n" +
                "		country_code_	VARCHAR2(10);\r\n	BEGIN " + lsDetailedAddressDbStmt + ";  " + Vis.StrSubstitute(lsAddressLayoutsDbStmt, ":i_hWndFrame.frmTaxOfficeInfoAddress.sCountryCode", "country_code_") + ";  " + Vis.StrSubstitute(lsLovReferenceDbStmt, ":i_hWndFrame.frmTaxOfficeInfoAddress.sCountryCode", 
					"country_code_") + "; " + ":i_hWndFrame.frmTaxOfficeInfoAddress.sCountryCode := country_code_;\r\n	END;";
				DbPLSQLBlock(cSessionManager.c_hSql, lsStmt);
			}
        ...
        public virtual SalNumber UM_DetailedAddress(SalNumber nWhat)
        {
            ...
            using (new SalContext(this))
            {
                if (nWhat == Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire)
                {
                    ...
                    if (!(Ifs.Fnd.ApplicationForms.Var.Security.IsDataSourceAvailable(Pal.GetActiveInstanceName("dlgDetailedTaxOfficeAddress"))))
                    {
                        return false;
                    }
        ...
        public virtual SalNumber ShowDetailedAddress()
        {
            ...
            using (new SalContext(this))
            {
                if (Ifs.Fnd.ApplicationForms.Var.Security.IsDataSourceAvailable(Pal.GetActiveInstanceName("dlgDetailedTaxOfficeAddress")))
                {
                    if (sDetailAddress == "TRUE")
                    {
                        sItemNames[0] = "TAX_OFFICE_ID";
                        hWndItems[0] = dfsTaxOfficeId;
                        ...
                        dlgDetailedTaxOfficeAddress.ModalDialog(i_hWndFrame);
            ...
            using (new SalContext(this))
            {
                ...
                if (Ifs.Fnd.ApplicationForms.Var.Security.IsMethodAvailable("Enterp_Address_Country_API.Get_Detailed_Address"))
                {
                    using (SignatureHints hints = SignatureHints.NewContext())
                    {
                        ...
                        DbPLSQLBlock(cSessionManager.c_hSql, ":i_hWndFrame.frmTaxOfficeInfoAddress.sDetailAddress := \r\n" +
                            "                                               		&AO.Enterp_Address_Country_API.Get_Detailed_Address(\r\n" +
                            "                                               		&AO.ISO_Country_Api.Encode(\r\n" +
                            "				:i_hWndFrame.frmTaxOfficeInfoAddress.cmbCountry ))");
                    }
        ...
        public virtual SalNumber CheckAddress1()
        {
            ...
            using (new SalContext(this))
            {
                using (SignatureHints hints = SignatureHints.NewContext())
                {
                    hints.Add("Tax_Office_Info_Address_API.Get_Street", System.Data.ParameterDirection.Input, System.Data.ParameterDirection.Input);
                    hints.Add("Tax_Office_Info_Address_API.Get_House_No", System.Data.ParameterDirection.Input, System.Data.ParameterDirection.Input);
                    if (!(DbPLSQLBlock(cSessionManager.c_hSql, "BEGIN\r\n" +
                        " :i_hWndFrame.frmTaxOfficeInfoAddress.sStreet := &AO.Tax_Office_Info_Address_API.Get_Street(:i_hWndFrame.frmTaxOfficeInfoAddress.dfsTaxOfficeId,\r\n" +
                        "								                 :i_hWndFrame.frmTaxOfficeInfoAddress.cmbAddressId.i_sMyValue);\r\n" +
                        " :i_hWndFrame.frmTaxOfficeInfoAddress.sHouseNo := &AO.Tax_Office_Info_Address_API.Get_House_No(:i_hWndFrame.frmTaxOfficeInfoAddress.dfsTaxOfficeId,\r\n" +
                        "									:i_hWndFrame.frmTaxOfficeInfoAddress.cmbAddressId.i_sMyValue);\r\n END;")))
                    {
                        return false;
                    }
                }
                sAddress1 = dfsAddress1.Text;
                if (sStreet == SalString.Null && sHouseNo == SalString.Null && sAddress1 == SalString.Null)
                {
                    sAddress1 = " ";
                }
                sNewAddress1 = sStreet + " " + sHouseNo;
                if (sAddress1 != sNewAddress1)
                {
		...
		private void frmTaxOfficeInfoAddress_WindowActions(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			switch (e.ActionType)
			{
				case Ifs.Fnd.ApplicationForms.Const.PM_DataRecordNew:
					this.frmTaxOfficeInfoAddress_OnPM_DataRecordNew(sender, e);
					break;
				
				case Ifs.Fnd.ApplicationForms.Const.PM_DataRecordDuplicate:
					this.frmTaxOfficeInfoAddress_OnPM_DataRecordDuplicate(sender, e);
					break;
				
				case Sys.SAM_Create:
					this.frmTaxOfficeInfoAddress_OnSAM_Create(sender, e);
					break;
				
				
				case Ifs.Fnd.ApplicationForms.Const.PM_DataRecordRemove:
					this.frmTaxOfficeInfoAddress_OnPM_DataRecordRemove(sender, e);
					break;
				
                case Const.PAM_DetailAddress:
                    this.frmTaxOfficeInfoAddress_OnPAM_DetailAddress(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataRecordPaste:
                    this.frmTaxOfficeInfoAddres_OnPM_DataRecordPaste(sender, e);
                    break;
			}
			#endregion
		}
		...
		private void frmTaxOfficeInfoAddress_OnPM_DataRecordNew(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
			if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataRecordNew, Sys.wParam, Sys.lParam)) 
			{
				...
				{
					Sal.SendMsg(this.cmbCountry, Ifs.Fnd.ApplicationForms.Const.PM_DataItemValueSet, 1, ((SalString)frmTaxOfficeInfo.FromHandle(this.i_hWndParent).cmbCountry.Text).ToHandle());
		...
		/// </summary>
		/// <param name="message"></param>
		private void frmTaxOfficeInfoAddress_OnPM_DataRecordDuplicate(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
		...
		/// </summary>
		/// <param name="message"></param>
		private void frmTaxOfficeInfoAddress_OnSAM_Create(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
		...
		/// </summary>
		/// <param name="message"></param>
		private void frmTaxOfficeInfoAddress_OnPM_DataRecordRemove(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
			if (Sys.wParam == Ifs.Fnd.ApplicationForms.Const.METHOD_Execute) 
        ...
        /// </summary>
        /// <param name="message"></param>
        private void frmTaxOfficeInfoAddres_OnPM_DataRecordPaste(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        /// </summary>
        /// <param name="message"></param>
        private void frmTaxOfficeInfoAddress_OnPAM_DetailAddress(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        #region ChildTable - tblAddressType
            ...
            public SalString tblAddressType_sTempTaxOfficeId = "";
            #endregion
            ...
            /// </summary>
            /// <returns></returns>
            public virtual SalNumber tblAddressType_DataRecordPrepareNew()
            {
                #region Actions
                return true;
                #endregion
            ...
            public virtual SalBoolean CheckDefaultAddress()
            {
                ...
                using (new SalContext(tblAddressType))
                {
                    ...
                    tblAddressType_sTempTaxOfficeId = SalString.Null;
                    ...
                    while (true)
                    {
                        if (Sal.TblFindNextRow(tblAddressType, ref nCurrentRow, 0, 0))
                        {
                            ...
                            lsStmt = lsStmt + "&AO.TAX_OFFICE_ADDR_TYPE_API.Check_Def_Address_Exist (\r\n" +
                                                                                                    ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sTempAddressValidation[" + nIndex.ToString(0) + "] OUT,\r\n" +
                                                                                                    ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sValidFlag[" + nIndex.ToString(0) + "] OUT,\r\n" +
                                                                                                    ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sTempTaxOfficeId IN ,\r\n" +
                                                                                                    ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sDefAddress[" + nIndex.ToString(0) + "] IN ,\r\n" +
                                                                                                    ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sAddressTypeCodeArr[" + nIndex.ToString(0) + "] IN ,\r\n" +
                                                                                                    ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sObjId[" + nIndex.ToString(0) + "] IN,\r\n" +
                                                                                                    ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_dTempFromDate[" + nIndex.ToString(0) + "] IN,\r\n" +
                                                                                                    ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_dTempToDate[" + nIndex.ToString(0) + "]  IN );";
                            ...
                            tblAddressType_sTempTaxOfficeId = this.tblAddressType_colsTaxOfficeId.Text;
                    ...
                    sAddressTypeCodeMsg[1] = Properties.Resources.TEXT_TaxOficePartyType;
                    sAddressTypeCodeMsg[2] = this.tblAddressType_colsTaxOfficeId.Text;
                        ...
                        if (Sal.TblFindNextRow(tblAddressType, ref nCurrentRow, nRowCondition, 0))
                        {
                            ...
                            if (this.tblAddressType_colDefAddress.Text == "TRUE")
                            {
                                lsStmt = lsStmt + "&AO.TAX_OFFICE_ADDR_TYPE_API.Check_Def_Address_Exist (\r\n" +
                                                                                                        ...
                                                                                                        ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sAddressTypeCodeArr[" + nIndex.ToString(0) + "] IN ,\r\n" +
                                                                                                        ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sObjId[" + nIndex.ToString(0) + "] IN ,\r\n" +
                                                                                                        ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_dTempFromDate[" + ((SalNumber)0).ToString(0) + "] IN ,\r\n" +
                                                                                                        ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_dTempToDate[" + ((SalNumber)0).ToString(0) + "] IN );";
                                tblAddressType_sAddressId[nIndex] = this.tblAddressType_colsAddressId.Text;
                                tblAddressType_sAddressTypeCodeArr[nIndex] = this.tblAddressType_colsAddressTypeCode.Text;
                                tblAddressType_sDefAddress[nIndex] = this.tblAddressType_colDefAddress.Text;
                                tblAddressType_sObjId[nIndex] = this.tblAddressType.__colObjid.Text;
            ...
            public virtual SalBoolean CheckLastAddressType()
            {
                ...
                using (new SalContext(this))
                {
                    ...
                    if (nCountRows == 0)
                    {
                        ...
                        sMessage[1] = Properties.Resources.TEXT_TaxOficePartyType.ToLower();
                        sMessage[2] = this.tblAddressType_colsTaxOfficeId.Text;
            ...
            public virtual SalBoolean RemoveDefaultAddress()
            {
                ...
                using (new SalContext(tblAddressType))
                {
                    ...
                    while (nIndex <= nCount)
                    {
                        ...
                        {
                            lsStmt = lsStmt + "&AO.TAX_OFFICE_ADDR_TYPE_API.Check_Def_Addr_Temp (:i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sTempTaxOfficeId IN ,\r\n" +
                                                                                                ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sAddressTypeCodeArr[" + nIndex.ToString(0) + "] IN,\r\n" +
                                                                                                ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sDefAddress[" + nIndex.ToString(0) + "] IN ,\r\n" +
                                                                                                ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_sObjId[" + nIndex.ToString(0) + "] IN ,\r\n" +
                                                                                                ":i_hWndFrame.frmTaxOfficeInfoAddress.tblAddressType_dTempFromDate[" + nIndex.ToString(0) + "] IN ,\r\n" +
            ...
            public virtual SalNumber tblAddressType_DataSourcePrepareKeyTransfer(SalString sWindowName)
            {
                ...
                using (new SalContext(this))
                {
                    if (sWindowName == Pal.GetActiveInstanceName("frmHistoryLog"))
                    {
                        ...
                        sItemNames[1] = "TAX_OFFICE_ADDR_TYPE_CODE";
                        hWndItems[1] = tblAddressType_colsAddressTypeCodeDb;
                        sItemNames[2] = "TAX_OFFICE_ID";
                        hWndItems[2] = tblAddressType_colsTaxOfficeId;
                    }
                    else
                    {
                        return ((cDataSource)this.tblAddressType).DataSourcePrepareKeyTransfer(sWindowName);
                    }
                    Ifs.Fnd.ApplicationForms.Var.DataTransfer.Init("TaxOfficeAddrType", tblAddressType, sItemNames, hWndItems);
            ...
            private void tblAddressType_DataRecordPrepareNewEvent(object sender, FndReturnEventArgsSalBoolean e)
            {
                e.Handled = true;
                e.ReturnValue = tblAddressType_DataRecordPrepareNew();
            }
        ...
        #region ChildTable - tblCommMethod
            ...
            public virtual SalBoolean tblCommMethod_DataRecordExecuteNew(SalSqlHandle hSql)
            {
                ...
                using (new SalContext(this))
                {
                    ...
                    tblCommMethod.colsIdentity.Text = SalString.FromHandle(this.dfsTaxOfficeId.EditDataItemValueGet());
        ...
        private void menuItem_View_Execute(object sender, Fnd.Windows.Forms.FndCommandExecuteEventArgs e)
        {
        ...
        private void menuItem_View_Inquire(object sender, Fnd.Windows.Forms.FndCommandInquireEventArgs e)
        {
