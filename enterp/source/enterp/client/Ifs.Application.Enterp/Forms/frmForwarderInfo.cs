...
// 140704   Hawalk  Bug 116673, Changed view of tblOurId from FORWARDER_INFO_OUR_ID to FORWARDER_INFO_OUR_ID_FIN_AUTH (user-company authorization rests
...
namespace Ifs.Application.Enterp
{
	...
	/// <summary>
	/// </summary>
	[FndWindowRegistration("FORWARDER_INFO", "ForwarderInfo", FndWindowRegistrationFlags.HomePage)]
	public partial class frmForwarderInfo : cMasterDetailTabFormWindow
	{
		...
		/// <returns></returns>
		public new static frmForwarderInfo FromHandle(SalWindowHandle handle)
		{
			return ((frmForwarderInfo)SalWindow.FromHandle(handle, typeof(frmForwarderInfo)));
		}
		#endregion
		...
		public new SalBoolean FrameStartupUser()
		{
			...
			{
				Sal.SetWindowText(i_hWndFrame, Properties.Resources.HEAD_DetailForwarderInfo);
                return true;
			}
			#endregion
		}
			...
			using (new SalContext(this))
			{
				...
				using(SignatureHints hints = SignatureHints.NewContext())
				{
					...
					DbPLSQLBlock(cSessionManager.c_hSql, sFullFormName + ".dfsExist :=\r\n" +
						"                                 &AO.Association_Info_API.Association_No_Exist( " + sFullFormName + ".dfsAssociationNo, 'FORWARDER' )");
				}
		...
		public new SalBoolean DataSourceSaveCheckOk()
		{
			...
			using (new SalContext(this))
			{
				...
				{
					if (Sal.SendMsg(frmForwarderInfoAddress.FromHandle(hWndAddress).tblCommMethod, Ifs.Fnd.ApplicationForms.Const.PM_DataSourceIsDirty, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire, Sys.lParam)) 
					{
						if (!(frmForwarderInfoAddress.FromHandle(hWndAddress).tblCommMethod.CheckDefault())) 
						{
							return false;
						}
					}
				...
				{
					if (Sal.SendMsg(frmForwarderInfo.FromHandle(i_hWndFrame).TabAttachedWindowHandleGet(picTab.FindName("Name2")), Ifs.Fnd.ApplicationForms.Const.PM_DataSourceIsDirty, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire, Sys.lParam)) 
					{
						if (!(frmForwarderInfoCommMethod.FromHandle(hWndComMethod).tblCommMethod.CheckDefault())) 
						{
							return false;
						}
					}
		...
		public new SalBoolean DataSourceSaveCheck()
		{
			...
			using (new SalContext(this))
			{
				...
				if (sAddressTabActive == "TRUE") 
				{
					if (((bool)Sal.SendMsg(frmForwarderInfoAddress.FromHandle(hWndAddress).tblAddressType, Ifs.Fnd.ApplicationForms.Const.PM_DataSourceIsDirty, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire, Sys.lParam)) || (Sal.QueryFieldEdit(frmForwarderInfoAddress.FromHandle(hWndAddress).dfdValidFrom) || 
					Sal.QueryFieldEdit(frmForwarderInfoAddress.FromHandle(hWndAddress).dfdValidTo))) 
					{
						if (!(frmForwarderInfoAddress.FromHandle(hWndAddress).CheckDefaultAddress())) 
						{
							return false;
						}
                        if (!(frmForwarderInfoAddress.FromHandle(hWndAddress).CheckLastAddressType()))
                        {
                            return false;
                        }
					}
        ...
        public virtual new SalNumber TabActivateFinish(SalWindowHandle hWnd, SalNumber nTab)
        {
            ...
            using (new SalContext(this))
            {
                ...
                if (sTabName == "Name2")
                {
                    ...
                    Sal.SetFocus(frmForwarderInfoCommMethod.FromHandle(hWndInfoCommMethod).tblCommMethod);
                }
            }
            return retVal;
            #endregion
		...
		private void frmForwarderInfo_WindowActions(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			switch (e.ActionType)
			{
				case Sys.SAM_Create:
					this.frmForwarderInfo_OnSAM_Create(sender, e);
					break;
				
				case Ifs.Fnd.ApplicationForms.Const.PM_DataSourcePopulate:
					this.frmForwarderInfo_OnPM_DataSourcePopulate(sender, e);
					break;
				
				
				case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceSave:
					this.frmForwarderInfo_OnPM_DataSourceSave(sender, e);
					break;
				
                
                case Ifs.Fnd.ApplicationForms.Const.PM_DataRecordDuplicate:
                    this.frmForwarderInfo_OnPM_DataRecordDuplicate(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataRecordPaste:
                    this.frmForwarderInfo_OnPM_DataRecordPaste(sender, e);
                    break;
		...
		private void frmForwarderInfo_OnSAM_Create(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
		...
		/// </summary>
		/// <param name="message"></param>
		private void frmForwarderInfo_OnPM_DataSourcePopulate(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
		...
		/// </summary>
		/// <param name="message"></param>
		private void frmForwarderInfo_OnPM_DataSourceSave(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
				...
				if (Sys.wParam == Ifs.Fnd.ApplicationForms.Const.METHOD_Execute) 
				{
					if (!(this.bAddressType)) 
					{
						...
						if (this.bTabName1Edited) 
						{
							...
							Sal.SendMsg(this.TabAttachedWindowHandleGet(this.picTab.FindName("Name1")), Ifs.Fnd.ApplicationForms.Const.PM_DataSourceRefresh, Sys.wParam, Sys.lParam);
						}
					}
					this.bAddressType = false;
				}
        ...
        private void frmForwarderInfo_OnPM_DataRecordDuplicate(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void frmForwarderInfo_OnPM_DataRecordPaste(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
		...
		/// <param name="sender">The source of the event.</param>
		/// <param name="e">An <see ref="PPJ.Runtime.WindowActionsEventArgs"/> that contains the event data.</param>
		private void cmbForwarderId_WindowActions(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			switch (e.ActionType)
			{
				case Ifs.Fnd.ApplicationForms.Const.PM_DataItemNew:
					this.cmbForwarderId_OnPM_DataItemNew(sender, e);
					break;
			}
			#endregion
		}
		...
		/// </summary>
		/// <param name="message"></param>
		private void cmbForwarderId_OnPM_DataItemNew(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
			Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemNew, Sys.wParam, Sys.lParam);
			this.cmbForwarderId.EditDataItemSetEdited();
			e.Return = true;
			return;
			#endregion
		}
